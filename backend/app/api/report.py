...
# ---------------------------------------------------------------------------
# 中文字体处理（跨平台）
# 报告里的图表(matplotlib)与 PDF 正文(reportlab)都需要真实的中文字体文件。
# 开发机为 Windows，可直接用 SimSun/SimHei；但 Linux 服务器上这些字体默认不存在，
# 若不做探测会静默退化成方框/乱码（原实现有 try/except 兜底，不报错但中文丢失）。
# 因此这里按平台常见路径依次探测，第一个可用的字体同时用于图表与 PDF。
# 部署时也可把字体文件放到 backend/fonts/ 目录，随项目一起下发。
# ---------------------------------------------------------------------------
_FONT_DIR = os.path.join(settings.BASE_DIR, "fonts")

# 候选字体文件，按优先级排列。.ttc 为字体集合，reportlab 注册时需指定 subfontIndex；
# OTF/CFF 字体 reportlab 不支持，注册会失败并自动尝试下一个候选。
_CJK_FONT_CANDIDATES = [
    # 1) 随项目下发的字体（优先级最高，部署最可控）
    ...
    # 3) Linux 服务器：手工拷入的 Windows 字体
    "/usr/share/fonts/truetype/win/simhei.ttf",
    "/usr/share/fonts/truetype/win/simsun.ttc",
    # 4) Linux 服务器：发行版自带的开源中文字体
...
def _iter_cjk_font_files():
    """按优先级依次产出实际存在的中文字体文件路径。"""
    seen = set()
...
def configure_matplotlib_chinese():
    ...
    优先把探测到的字体文件注册进 matplotlib 字体管理器，再按其族名使用；
    同时保留一批常见字体名作为兜底（覆盖字体已装但不在候选路径中的情况）。
    字体注册较慢，用模块级标记保证只执行一次。
    """
    ...
    families = []
    for path in _iter_cjk_font_files():
        try:
            font_manager.fontManager.addfont(path)
            families.append(font_manager.FontProperties(fname=path).get_name())
        except Exception as e:
            # 个别 .ttc 集合字体 matplotlib 解析失败属正常情况，跳过即可
            print(f"Warning: Failed to load matplotlib font {path}: {e}")

    if not families:
        print("Warning: No CJK font file found, chart Chinese text may render as boxes.")

    families += ['SimHei', 'Microsoft YaHei', 'WenQuanYi Zen Hei',
...
def register_pdf_chinese_font() -> str:
    """为 reportlab 注册中文字体，返回可用的字体名。

    全部候选都失败时返回 Helvetica（PDF 中文会丢失），并打印告警。
    结果缓存，避免重复注册同名字体。
    """
    ...
    for path in _iter_cjk_font_files():
        try:
            ...
            print(f"Warning: Failed to register PDF font {path}: {e}")

    print("Warning: No CJK font available, PDF Chinese characters will be missing.")
...
def extract_workflow_data(workflow_data: Dict, user_dir: str):
    ...
    algo_nodes = [n for n in nodes if n.get("type") == "algoNode" and n.get(
        "data", {}).get("category") == "Model"]
...
def generate_charts(algo_results: List[Dict]):
    ...
    if clus_results:
        ...
        sil_values = [r["result"].get("silhouette_score", -1)
                      for r in clus_results]
...
def create_docx(workflow_name: str, data_filename: str, eda_stats: Dict, algo_results: List[Dict], charts: Dict):
    ...
    if not algo_results:
        ...
        clf_results = [
            r for r in algo_results if r["type"] == "classification"]
        if clf_results:
...
def create_pdf(workflow_name: str, data_filename: str, eda_stats: Dict, algo_results: List[Dict], charts: Dict):
    ...
    # Register Font
    # 跨平台探测中文字体，Linux 服务器上无 SimSun/SimHei 时会自动回落到开源中文字体
    font_name = register_pdf_chinese_font()

    styles = getSampleStyleSheet()
    # Define Chinese Style
    cn_title_style = ParagraphStyle(
        name='CNTitle', parent=styles['Title'], fontName=font_name, fontSize=18, leading=22)
    cn_h1_style = ParagraphStyle(
        name='CNHeading1', parent=styles['Heading1'], fontName=font_name, fontSize=16, leading=20, spaceBefore=12, spaceAfter=6)
    cn_h2_style = ParagraphStyle(
        name='CNHeading2', parent=styles['Heading2'], fontName=font_name, fontSize=14, leading=18, spaceBefore=10, spaceAfter=6)
    cn_body_style = ParagraphStyle(
        name='CNBody', parent=styles['Normal'], fontName=font_name, fontSize=10, leading=14)

    # Title
    elements.append(Paragraph(f'算法分析报告 - {workflow_name}', cn_title_style))
    elements.append(Spacer(1, 12))
    elements.append(Paragraph(
        f'生成时间: {pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")}', cn_body_style))
    elements.append(
        Paragraph(f'数据来源: {data_filename or "未指定"}', cn_body_style))
    ...
    if eda_stats:
        elements.append(
            Paragraph(f"总行数: {eda_stats.get('rows', 0)}", cn_body_style))
        elements.append(Spacer(1, 12))
    ...
    if not algo_results:
        ...
        if 'classification_radar' in charts:
            ...
            elements.append(
                RLImage(charts['classification_radar'], width=400, height=300))
            elements.append(Spacer(1, 12))

        if 'regression_bar' in charts:
            elements.append(Paragraph('回归模型误差对比图：', cn_body_style))
            elements.append(
                RLImage(charts['regression_bar'], width=400, height=250))
            elements.append(Spacer(1, 12))

        if 'clustering_bar' in charts:
            elements.append(Paragraph('聚类模型轮廓系数对比图：', cn_body_style))
            elements.append(
                RLImage(charts['clustering_bar'], width=400, height=250))
            elements.append(Spacer(1, 12))
...
async def export_report(
    ...
    try:
        ...
        # 2. Extract Data
        data_filename, eda_stats, algo_results = extract_workflow_data(
            workflow_data, user_dir)
        ...
        if request.format == "docx":
            file_stream = create_docx(
                request.workflow_name, data_filename, eda_stats, algo_results, charts)
            media_type = "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
            filename = f"{request.workflow_name}_report.docx"
        elif request.format == "pdf":
            file_stream = create_pdf(
                request.workflow_name, data_filename, eda_stats, algo_results, charts)
